...
public class StringOperations {
    ...
    private static final String PATH_TO_STOPWORD_FILE = "./oaei-resources/stopwords/english_stopwords.txt";
    ...
    public static String[] removeFreeFloatingGenitiveS(String[] tokenized) {
        int count = 0;
        for (String s : tokenized) {
            if (s.equals("s")) count++;
        }

        String[] result = new String[tokenized.length - count];
        int currentPosition = 0;
        for (String s : tokenized) {
            if (!s.equals("s")) {
                result[currentPosition] = s;
                currentPosition++;
            }
        }
        return result;
